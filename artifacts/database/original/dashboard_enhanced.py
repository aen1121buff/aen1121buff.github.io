"""
Enhanced CS 340 Grazioso Salvare dashboard for CS 499 Milestone Three.

This dashboard keeps the original filtering purpose but adds a ranked rescue
candidate score. The ranking algorithm helps users see the strongest candidates
first instead of only seeing unsorted matching database records.
"""
...
def update_table(rescue_type):
    ...
    count_message = f"Showing {len(dataframe)} records for {label}. Ranked results show strongest candidates first."
